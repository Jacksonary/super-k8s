...
import { Button, Card, Empty, Input, Space, Pagination, Table, Typography, App as AntdApp } from "antd";
...
export default function CronJobs() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await api.listCronJobs(currentClusterId, currentNamespace || null);
      setCronjobs(data);
    } catch (err) {
  ...
  const columns: ColumnsType<CronJobInfo> = [
    {
      title: "Namespace",
      dataIndex: "namespace",
      key: "namespace",
    ...
    },
    {
      ...
      render: (n: number) => (
        <Text type={n > 0 ? "success" : "secondary"}>{n}</Text>
      ),
    },
    {
      title: "Last Schedule",
      dataIndex: "last_schedule_ms",
      key: "last_schedule_ms",
      width: 170,
      sorter: (a, b) => (a.last_schedule_ms ?? 0) - (b.last_schedule_ms ?? 0),
      render: (ms: number | null) =>
        ms != null ? formatTimestamp(ms) : <Text type="secondary">Never</Text>,
    },
    {
      title: "Age",
      dataIndex: "age_ms",
  ...
  const { columns: rcols, components } = useResizableColumns("cronjobs", columns);

  const shown = useMemo(
    () => cronjobs.filter((r) => (r.name ?? "").toLowerCase().includes(query.trim().toLowerCase())),
    [cronjobs, query],
  );
    ...
    >
      <div ref={tableRef} style={{ flex: 1, minHeight: 0, overflow: "hidden" }}>
        <Table<CronJobInfo>
          ...
          pagination={false}
          locale={{ emptyText: <Empty description="No cronjobs" /> }}
