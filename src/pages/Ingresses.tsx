...
export default function Ingresses() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await api.listIngresses(currentClusterId, currentNamespace || null);
      setIngresses(data);
    } catch (err) {
  ...
  const columns: ColumnsType<IngressInfo> = [
    {
      title: "Namespace",
      dataIndex: "namespace",
      key: "namespace",
    ...
    },
    {
      ...
      render: (cls: string | null) =>
        cls ? <Tag>{cls}</Tag> : <Text type="secondary">-</Text>,
    },
    {
      title: "Hosts",
      dataIndex: "hosts",
  ...
  const { columns: rcols, components } = useResizableColumns("ingresses", columns);

  const shown = useMemo(
    () => ingresses.filter((r) => r.name.toLowerCase().includes(query.trim().toLowerCase())),
    [ingresses, query],
  );
    ...
    >
      <div ref={tableRef} style={{ flex: 1, minHeight: 0, overflow: "hidden" }}>
        <Table<IngressInfo>
          ...
          pagination={false}
          locale={{ emptyText: <Empty description="No ingresses" /> }}
